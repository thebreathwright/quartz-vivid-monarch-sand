...
- raw sprite art must come from built-in `imagine_image`, not Three.js, Canvas, SVG, HTML/CSS drawing, PIL shape drawing, procedural geometry, placeholder primitives, or code-rendered screenshots
...
- Pass the reference to built-in `imagine_image` as an `asset_id` (a sandbox file needs `imagine_create_asset` first), and `read_file` it so you can see it too. Do not assume a path string in the prompt is a visual input.
...
When using a layout guide, register it with `imagine_create_asset` and pass its `asset_id` in `asset_ids` on the `imagine_image` call (file path on the older split tools), `read_file` it so you can see it yourself, and write:

```text
Use the provided layout guide image as a layout-only reference. Use it only to understand the rows, columns, equal invisible frame slots, centering, spacing, and safe padding. Do not reproduce the guide: no visible boxes, no safe-area rectangles, no center marks, no labels, no borders, no guide background.
```

...
metadata:
  short-description: "2D sprite sheets: imagine_image + magenta chroma postprocess"
...
| Image tools | `imagine_image` + `imagine_view_media` / `imagine_create_asset`, or the older `generate_image` / `edit_image` where those are what the tool list has (see **`imagine`** skill for prompt craft) |
| Inspect images | `read_file` on the PNG path (not Codex view_image) |
| Generated image path | `imagine_image` returns an `asset_id`; call `imagine_view_media` to download it into the sandbox, then copy that path into your run dir before processing |
...
via `imagine_video`), **`building-games`** (game loop / integration).
...
- Use built-in `imagine_image` for every raw image.
- Do not create raw sprite art with Three.js, Canvas, SVG, HTML/CSS drawing, PIL shape drawing, procedural geometry, placeholder primitives, or code-rendered screenshots. Runtime code may display finished generated assets, and scripts may make layout guides or postprocess generated images, but requested sprite art must originate from built-in `imagine_image`.
- When the user provides or implies a visual reference, pass that reference to `imagine_image` as an `asset_id` (register a sandbox file with `imagine_create_asset` first). Also call `read_file` on a local reference so you can see it; never rely on a filesystem path inside the prompt as the visual reference.
...
- Wire the reference into the call: pass its `asset_id` in `asset_ids` (freshly generated references already have one; register local files with `imagine_create_asset` first; file paths on the older split tools). Also `read_file` local references so you can see them.
...
- Do not ask `imagine_image` to generate unrelated action rows in one raw sheet, such as `row 1 idle, row 2 run, row 3 shoot, row 4 jump`, for a controllable hero or main character.
- Do not ask `imagine_image` to generate raw single-row action strips such as `1x4 idle`, `1x4 run`, `1x4 shoot`, or `1x4 jump` for a controllable hero, character, creature, NPC, enemy, summon, or animated prop.
...
If a layout guide is useful, generate one before calling built-in `imagine_image`:
...
Then register the guide with `imagine_create_asset` and pass its `asset_id` in `asset_ids` on the `imagine_image` call (file path on the older split tools) — a guide that is only "visible in conversation" never reaches the image model. Also `read_file` it so you can see the geometry. Tell `imagine_image` to use it only for invisible slot count, spacing, centering, and safe padding. The output must not reproduce guide boxes, safe-area rectangles, center marks, labels, borders, or guide background.
...
- keep the returned `asset_id`, then call `imagine_view_media` to download the
  image into the sandbox
- copy that file into the working output folder as `raw-sheet.png` (or similar)
- keep the original generated image in place
- to run a further Imagine edit on a **postprocessed** PNG, register it first
  with `imagine_create_asset` and pass the new `asset_id`

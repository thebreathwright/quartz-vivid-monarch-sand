...
name: generate2dmap
description: >
  Generate production-oriented 2D game maps with `imagine_image`: RPG/top-down maps,
...
| Item | Value |
| --- | --- |
| Skill dir | `.grok/skills/generate2dmap/` |
| Scripts | `python3 .grok/skills/generate2dmap/scripts/<script>.py …` |
| Image tools | `imagine_image` + `imagine_view_media` / `imagine_create_asset` (see **`imagine`**) |
| Inspect images | `read_file` on PNG paths (not Codex view_image) |
| Generated image path | `imagine_image` → `asset_id` → `imagine_view_media` → copy into `assets/map/` |
| Python deps | Pillow (preinstalled) |
| Prop sprites | Prefer **`generate2dsprite`** for character/FX sprites; prop packs use this skill's extract script |
...
Related: **`generate2dsprite`**, **`building-games`**, **`imagine`**.
...
This skill is image-generation-first for visual assets. Use built-in `imagine_image` as the default creative art source for base maps, in-world reference mockups, dressed references, stage references, prop sheets, prop sprites, tileset art, parallax layers, battle backgrounds, and other visible map assets.
...
1. Save the base/background image first and keep its `asset_id`. If it only exists as a sandbox file, register it with `imagine_create_asset` to get an `asset_id`.
2. Pass that `asset_id` in `asset_ids` on the next `imagine_image` call — this is what actually hands the reference to the image model. Also call `read_file` on the saved file so you can see it yourself. (On the older split tools, pass the file path instead of an `asset_id`.)
3. In the next `imagine_image` prompt, explicitly say to use the provided reference image as the visual reference.
...
Do not rely on a path string, filename, prompt wording like "based on the map", or the image merely being visible in conversation as the reference handoff. If the base/background is not wired into the call as an `asset_id` (or a file path on the older split tools), stop and register/pass it before generating the dressed reference or stage reference.
...
- `visual_asset_source`: imagine_image | existing_assets | procedural_placeholder
...
- Use `imagine_image` as the visual asset source.
...
2. Choose the pipeline axes.
   ...
   - Select `visual_asset_source`. Default to `imagine_image`; use `existing_assets` only when the project already has suitable art; use `procedural_placeholder` only when explicitly requested.
   - Treat `hybrid` as a result of combining axes, not as a primary category.

3. Produce assets.
   - Write the creative prompts manually and use built-in `imagine_image` for visible map art unless the user explicitly chose existing assets or procedural placeholders.
   - For baked raster maps, generate one background with built-in `imagine_image`, or edit/use an existing image when supplied, then add optional collision/zones metadata.
...
2. Hand the base to `imagine_image` as a real reference: pass its `asset_id` in `asset_ids` (register a local file with `imagine_create_asset` first). Also `read_file` it so you can see it; do not rely on a path string or prompt text as the reference. On the older split tools, pass the file path directly.
...
4. For each visible runtime object, choose exactly one asset strategy:
   - generate a separate transparent asset with `$generate2dsprite` or direct `imagine_image`
...
5. For every object/prop generation that must match the map style, pass the base/background and/or reference mockup `asset_id`s in `asset_ids` (register local files with `imagine_create_asset` first; file paths on the older split tools), and say in the prompt that the provided reference images are the visual context. The generated asset must match the original map style and correspond to an object visible in the reference mockup.
...
2. Hand the background to `imagine_image` as a real reference: pass its `asset_id` in `asset_ids` (register a local file with `imagine_create_asset` first). Also `read_file` it so you can see it; do not rely on a path string or prompt text as the reference. On the older split tools, pass the file path directly.

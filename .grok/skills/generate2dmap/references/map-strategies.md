...
1. Save the base/background image first and keep its `asset_id` (register a local file with `imagine_create_asset`).
2. Pass that `asset_id` in `asset_ids` on the reference-mockup `imagine_image` call (file path on the older split tools). Also call `read_file` on the saved image so you can see it yourself.
...
Do not rely on filenames, paths, vague phrasing such as "based on this map", or the image merely being visible in conversation. If the base/background is not wired into the call as an `asset_id` (or file path on the older split tools), stop and register/pass it before generating the dressed reference or stage reference.

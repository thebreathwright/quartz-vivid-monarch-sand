...
2. Hand the base to built-in `imagine_image` as a real reference: pass its `asset_id` in `asset_ids` (register a local file with `imagine_create_asset` first). Also call `read_file` so you can see it. Do not expect a filesystem path in the prompt, or the image merely being visible in conversation, to work as the visual reference. On the older split tools, pass the file path directly.

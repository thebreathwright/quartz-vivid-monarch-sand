...
| Denser locomotion from video | **`video2dsprite`** — base still on **`#FF00FF`** → `imagine_video` → skill scripts (ffmpeg + chroma) |
...
2. **Animate.** `imagine_video` from the base: one clear motion, in place,
...
Fall back to keyframe-by-keyframe `imagine_image` (still on `#FF00FF` when

...
description: >
  ...
  imagine_image base → imagine_video (6s/10s run-in-place) → ffmpeg
  ...
  "video to sprite", "imagine_video sprite", "dense walk cycle", "smooth run
  animation from video".
metadata:
  short-description: "Video→dense sprites (imagine_video + chroma postprocess)"
...
| Video tools | `imagine_video` — animate one base `asset_id` (verify present) |
...
```text
base still → imagine_video (in-place motion) → extract frames → chroma key → sample/normalize → strip / grid / GIF
```
...
as `imagine_image`, or as `generate_image` / `edit_image`; video as `imagine_video`
or `generate_video`. Use whichever pair your tool list has — the split tools
take filesystem paths where this skill says `asset_id`, so the
`imagine_create_asset` / `imagine_view_media` steps don't apply. Stop and
...
2. **Still → video, never text-to-video alone.** Stage frame 1 as a clean still with `imagine_image` (from a prompt, or from a reference `asset_id`). Then call `imagine_video` with that still's `asset_id`.
...
5. **Do not invent art with PIL/Canvas.** Base art comes from `imagine_image` or a user/local still. Scripts only postprocess.
...
- **A. Existing sprite:** read the image, composite onto solid `#FF00FF` if needed, then `imagine_create_asset` so Imagine can use it
- **B. New character:** `imagine_image` with solid magenta background, full body, side view, centered
- **C. Match reference:** `imagine_image` with the reference `asset_id`, moving it onto magenta and preserving identity
...
Download it with `imagine_view_media` and save as
`<out_dir>/base/<name>-base.png`; keep the base `asset_id` for the video call.

Write the exact image prompt into `prompt-used.txt`.

### 3. Animate with `imagine_video`

Call Grok `imagine_video`:

- `asset_ids`: `[<base still asset_id>]` (not a filesystem path)
...
Download the returned video asset with `imagine_view_media` and copy it to
...
- Background: **#FF00FF**
- Prefer single-asset `imagine_video` over multi-ref (compose multi-ref with `imagine_image` first if needed)
